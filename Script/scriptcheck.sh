rm -f s1_result.txt s2_result.txt s1s2.txt
		...
		if [[ $word == MDL16CS* ]]; then
			...
			if [[ $count -le 9 ]];then
				...
				if [[ $count == 18 ]];then
					...
					paste s1_result.txt s2_result.txt | awk '{print (($1 + $2)/2), $3}' > s1s2.txt
					paste c4b_regno.txt s1_result.txt s2_result.txt s1s2.txt | column -s $'\t' -t >Y1_result.txt
				fi
			fi
...
echo "Do you want to delete intermediatory files??('y' for yes)"
read yn
if [[ $yn == y ]];then
	shopt -s extglob
	`rm -f -v !("result_MDL.pdf"|"S2_result_MDL.pdf"|"scriptcheck.sh"|"script.sh"|"c4b.txt"|"Y1_result.txt")`
fi	
xdg-open Y1_result.txt